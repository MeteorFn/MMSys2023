...
public:
    ...
    // Implementation
    void SortSession(std::multimap<double_t, fw::shared_ptr<SessionStreamController>> &sortmmap) override
    {
        SPDLOG_TRACE("");
        sortmmap.clear();
        for (auto &&sessionItor : m_dlsessionmap)
        {
            int64_t rtt = sessionItor.second->GetRtt().ToMilliseconds();
            double_t lossRate = sessionItor.second->GetLossRate();

            // weighted rtt and loss rate
            double_t score = 0.9 * rtt + lossRate * 100 * 0.1;
            sortmmap.emplace(score, sessionItor.second);
        }
    }
...
private:
    ...
    // Implementation
    std::multimap<double_t, fw::shared_ptr<SessionStreamController>> m_sortmmap;
